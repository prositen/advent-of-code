...
class Grove(object):
    ...
    def add(self, pos, delta):
        return tuple(map(operator.add, pos, delta))
    ...
    def step(self, steps=1):
        ...
        while True:
            ...
            for e in self.elves:
                if not any(self.add(e, d) in self.elves for d in self.delta):
                    new_elves[e].append(e)
                else:
                    for nb, mv in self.instructions:
                        if not any((self.add(e, n) in self.elves for n in nb)):
                            new_pos = self.add(e, mv)
